...
.section-title {
    ...
    border-left: 5px solid #0055b8; padding-left: 10px;
}
...
@media print {
    ...
    .report-container {
        ...
        box-shadow: none; margin: 0; padding: 15mm;
...
# ✅ 핵심 수정: apply로 문자열 DataFrame 새로 생성 (dtype 충돌 원천 차단)
MJ_ITEMS = {"열병합", "자가열전용"}

def format_table1_row(row):
    is_mj = row["용도"] in MJ_ITEMS
    return pd.Series({
        "용도": row["용도"],
        "전월(원/MJ)": fmt4(row["전월(원/MJ)"]) if is_mj else fmt2(row["전월(원/MJ)"]),
        "당월(원/MJ)": fmt4(row["당월(원/MJ)"]) if is_mj else fmt2(row["당월(원/MJ)"]),
        "증감(원/MJ)": fmt4(row["증감(원/MJ)"]) if is_mj else fmt2(row["증감(원/MJ)"]),
        "증감(%)":    fmt2(row["증감(%)"]),
    })
...
# ✅ 핵심 수정: apply로 문자열 DataFrame 새로 생성
...
<div class="section-caption">열병합 및 자가열전용 요금의 전월 대비 단가 변동을 나타냅니다.</div>"""
report_html += table1_disp.to_html(classes="styled-table", index=False)

report_html += f"""
<div class="section-title">2. 단위: 원/㎥</div>
<div class="section-caption">기준열량을 적용한 추정치로, 실제 열량 변동에 따라 차이가 발생할 수 있습니다.</div>
...
<div class="footer-note">
  ...
  https://cyber.daesungenergy.com/charge/solvAvgMJ</a><br/>
  ※ 문의: 대성에너지 마케팅팀
</div>"""
